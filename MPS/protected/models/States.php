...
class States extends CActiveRecord {
    ...
    public static function statesReport($arrInputs = array()) {
        ...
        try {
            ...
            if (isset($arrInputs['state_id']) && !empty($arrInputs['state_id'])) {
                $objDB->where('s.id=:id', array(':id' => $arrInputs['state_id']));
            }
    ...
    public static function FetchStateDetails($country_id) {
        ...
        $objectDB->from('states as ms');
        $objectDB->where('ms.countries_id=:countries_id', array(':countries_id' => $country_id));
