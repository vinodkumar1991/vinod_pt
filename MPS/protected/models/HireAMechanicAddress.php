...
class HireAMechanicAddress extends CActiveRecord {
    ...
    public static function updateHireAddress($arrHire, $intHire) {
        $objDB = Yii::app()->db->createCommand();
        $intUpdate = $objDB->update('hire_a_mechanic_address', $arrHire, 'hire_a_mechanic_id=:hireId', array(':hireId' => $intHire));
        return $intUpdate;
    }
