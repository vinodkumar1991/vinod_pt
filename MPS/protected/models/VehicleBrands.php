...
class VehicleBrands extends CActiveRecord
{
    ...
    public static function getVehicleBrands($intStatus = 1, $intVehicleType = NULL, $intVehicleBrands = NULL)
    {
        ...
        if (! empty($intVehicleBrands)) {
            $objectDB->where('v.id=:id', array(
                ':id' => $intVehicleBrands
            ));
            $arrVehicleBrands = $objectDB->queryRow();
        } elseif (! empty($intVehicleType)) {
            $objectDB->where('v.vehicle_types_id=:vehicleTypeId and v.status=:status', array(
                ':vehicleTypeId' => $intVehicleType,
                ':status' => $intStatus
            ));
            $arrVehicleBrands = $objectDB->queryAll();
        } else {
            $objectDB->where('v.status=:status', array(
                ':status' => $intStatus
            ));
            $arrVehicleBrands = $objectDB->queryAll();
        }
    ...
    public static function isBrandExist($strBrand)
    {
        ...
        $objectDB->where('vb.name=:name', array(
            ':name' => $strBrand
        ));
        $arrVehicleBrands = $objectDB->queryRow();
        return $arrVehicleBrands;
    ...
    public static function isBrandCodeExist($strCode)
    {
        ...
        $objectDB->select('vb.id');
        $objectDB->from('vehicle_brands as vb');
        $objectDB->where('vb.code=:code', array(
            ':code' => $strCode
        ));
    ...
    public static function brandsReport($arrInputs)
    {
        $arrBrands = array();
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->select('vb.id as brand_id,' . 'vb.name as brand_name,' . 'vb.code as brand_code,' . 'vb.logo as brand_logo,' . 'vb.status as brand_status,' . 'vt.name as vehicle_type,' . 'vt.id as vehicle_id');
        $objectDB->from('vehicle_brands as vb');
        $objectDB->join('vehicle_types as vt', 'vt.id = vb.vehicle_types_id');
        if (isset($arrInputs['status'])) {
            $objectDB->where('vb.status=:status', array(
                ':status' => $arrInputs['status']
            ));
        }
        if (isset($arrInputs['BrandID'])) {
            $objectDB->where('vb.id=:id', array(
                ':id' => $arrInputs['BrandID']
            ));
        }
    ...
    {
        $setValue = array(
            'status' => $arrInput['brand_status'],
            'last_modified' => $arrInput['created_date'],
            'ip_address' => $arrInput['ip_address']
        );

        $objectDB = Yii::app()->db->createCommand();
        $objectDB->update('vehicle_brands', $setValue, 'id=:id', array(
            'id' => $arrInput['brand_id']
        ));
    }
}
