...
class SelfVehicleImage extends CActiveRecord {
     ...
     */
    public static function create($arrVehicleImages, $intSelfVehicle = NULL) {
        $intVehImageId = NULL;
        if (!empty($intSelfVehicle)) {
            self::deleteSelfVehicleImages($intSelfVehicle);
        }
    ...
    public static function deleteSelfVehicleImages($intSelfVehicle) {
        $objCommand = Yii::app()->db->createCommand();
        $intDelete = $objCommand->delete('self_vehicles_images', 'self_vehicles_id=:selfVehicleId', array(':selfVehicleId' => $intSelfVehicle));
        return $intDelete;
    }
