...
class VehicleBrandModels extends CActiveRecord
{
    ...
    public static function getVehicleBrandModels($intVehicleBrandType, $intStatus = 1)
    {
        ...
        $objectDB->where('vbm.vehicle_brands_id=:vehicleBrandTypeId and vbm.status=:status', array(
            ':vehicleBrandTypeId' => $intVehicleBrandType,
            ':status' => $intStatus
        ));
    ...
    public static function isModelExist($strBrandModel)
    {
        ...
        $objectDB->from('vehicle_brand_models as vbm');
        $objectDB->where('vbm.name=:name', array(
            ':name' => $strBrandModel
        ));
    ...
    public static function isModelCodeExist($strBrandModelCode)
    {
        ...
        $objectDB->select('vbm.id');
        $objectDB->from('vehicle_brand_models as vbm');
        $objectDB->where('vbm.code=:code', array(
            ':code' => $strBrandModelCode
        ));
    ...
    public static function brandModelsReport($arrInputs)
    {
        ...
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->select('vbm.id as brand_model_id,' . 'vbm.name as brand_model_name,' . 'vbm.code as brand_model_code,' . 'vbm.image_path as brand_model_logo,' . 'vbm.status as brand_model_status,' . 'vb.name as brand_name,' . 'vb.id as brand_id,vt.id as vehicle_id,vt.name as vehicle_type_name,(CASE WHEN vbm.status = 1 THEN "Active" ELSE "Inactive" END) AS status_name');
        $objectDB->from('vehicle_brand_models as vbm');
        ...
        if (isset($arrInputs['status'])) {
            $objectDB->where('vbm.status=:status', array(
                ':status' => $arrInputs['status']
            ));
        }
        if (isset($arrInputs['ModelID'])) {
            $objectDB->where('vbm.id=:id', array(
                ':id' => $arrInputs['ModelID']
            ));
        }
    ...
    {
        $setValue = array(
            'status' => $arrInput['model_status'],
            'last_modified' => $arrInput['created_date'],
            'ip_address' => $arrInput['ip_address'],
            'name' => $arrInput['model_name'],
            'vehicle_brands_id' => $arrInput['edit_vehicle_brand_id']
        );
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->update('vehicle_brand_models', $setValue, 'id=:id', array(
            'id' => $arrInput['model_id']
        ));
    }
}
