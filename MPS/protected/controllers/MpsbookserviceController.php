...
class MpsbookserviceController extends Controller
{
	...
	public function actionFetchDataHomescreen()
	 ...
	 public function actionGethomescreenimages() {
       $strScreens = array();
       $intCode = 300;
       $arrScreens = MPSUPLOADS::getScreens();
       if (!empty($arrScreens)) {
           $intCode = 200;
       }
       $arrResponse = array('type' => 'success', 'code' => $intCode, 'data' => $arrScreens, 'success' => 'success');
       $strScreens = CommonFunctions::makeJson($arrResponse);
       echo $strScreens;
   }
	...
	public function actionLogin()//User login
		...
		{
			$arrMechanicStatus = array('status' => 0);
			$intCode = 300;
			$bookid=$_POST['bookid'];
			$intStatus = 1;
			$intUpdate = Yii::app()->db->createCommand('update MPS_PACKAGEWISE_AMT_DETAILS set serviceon="'.$intStatus.'" WHERE bookid="'.$bookid.'"')->execute(); 
			if($intUpdate){
				$intCode = 200;
				$arrMechanicStatus['status'] = 1;
			}
			$arrResponse = array('type'=>'success','code' => $intCode,'data' => $arrMechanicStatus,'message' =>'Mechanic Status');
	...
	public function actionFetchDropPickBoyDetails()
	{
			$bookid=$_POST['bookid'];
		    $arr=array();
			$fetchdata=Yii::app()->db->createCommand("SELECT distinct a.latitude,a.longitude,a.shop_id,a.del_nm,a.delv_id,a.contact,b.delv_status
								FROM delv_details as a
								LEFT JOIN MPS_PACKAGEWISE_AMT_DETAILS as b
								ON a.shop_id=b.mechp_id and a.delv_id=b.delv_id and b.bookid='$bookid'")->queryAll();
					$arr=array();
				...
				$pickBoyDetails=array("PickBoy Details"=>array($arr));
				echo json_encode($pickBoyDetails);
	}
	public function actionUpdateDropPickBoyStatus()
	{
			$bookid=$_POST['bookid'];
	 ...
	 public function actionFetchServiceCompletedStatus()
	{
		$arrResponse = array(); 
	    $arrServiceStatus = array('status' => 0);
		$bookid=$_POST['bookid'];
		$getcount2=Yii::app()->db->createCommand("select serviceon from  MPS_PACKAGEWISE_AMT_DETAILS where bookid='$bookid'")->queryAll(); 
		
		foreach($getcount2 as $value)
				{
					
					$status=$value['serviceon'];
				}
				if($status > 0)
				{
					$intCode = 200;
					$arrServiceStatus['status'] = $status;
				}
				
				$arrResponse = array('type'=>'success','code' => $intCode,'data' => $arrServiceStatus,'message' =>'Customer Service Details');	
		
		echo json_encode($arrResponse); 
	}
	public function actionFetchCustomerServiceDetails()
	{
		$arrCustomerServiceDet = array();
		$arrResponse = array();
		$intCode = 300;
		
		$bookid=$_POST['bookid'];
		
		$fetchdata=Yii::app()->db->createCommand("SELECT  servicetime,serviceon from MPS_PACKAGEWISE_AMT_DETAILS where bookid='$bookid'")->queryAll();
		if(!empty($fetchdata)){
			foreach($fetchdata as $value)
				{
					$arrCustomerServiceDet['Cust_Service'][]=$value;
					$status=$value['serviceon'];
				}
			...
			$arrResponse = array('type'=>'success','code' => $intCode,'data' => $arrCustomerServiceDet,'message' =>'Customer Service Details');	
			
			echo json_encode($arrResponse);
	}
	public function actionFetchDeliveruBoyServicetimeDetails()
	{
		$bookid=$_POST['bookid'];
		$fetchdata=Yii::app()->db->createCommand("SELECT  servicetime from MPS_PACKAGEWISE_AMT_DETAILS where bookid='$bookid'")->queryAll();
		$arr=array();
		foreach($fetchdata as $value)
				{
					$arr=$value;
				}
				$SeviceTime=array("ServiceTime Details"=>$arr);
				echo json_encode($SeviceTime); 
	}
	/* public function actionFetchServiceCompletedStatus()
	{
		$bookid=$_POST['bookid'];
...
ini_set('max_execution_time', 0);
		...
		$fetchdata=Yii::app()->db->createCommand("SELECT  mpa.userid,mpa.f_name,mpa.emailid,mpa.mobno,mpa.pickadrs,mpa.vehicle_type,mvd.lastserviceon,mpa.mechp_id,mpa.delv_status,mpa.servicetime FROM MPS_PACKAGEWISE_AMT_DETAILS as mpa,MPS_VEHICLE_DETAILS as mvd WHERE mpa.makes_id=mvd.makes_id and mpa.model_id=mvd.model_id and mpa.makes_id=$makes_id and mpa.model_id=$model_id and  mpa.bookid='$bookid'")->queryAll();
		foreach($fetchdata as $value)
				{
					$cust_details=$value;
					$delv_status=$value['delv_status'];
					$shopid=$value['mechp_id'];
					
				}
				 if($delv_status > 0)
				{
					
					 $fetchDeliveryBoydata=Yii::app()->db->createCommand("SELECT  a.latitude,a.longitude,a.shop_id,a.del_nm,a.delv_id,a.contact,b.delv_status FROM delv_details as a,MPS_PACKAGEWISE_AMT_DETAILS as b WHERE a.shop_id='$shopid' and b.bookid='$bookid' and a.shop_id=b.mechp_id and a.delv_id=b.delv_id")->queryAll();
		
				foreach($fetchDeliveryBoydata as $value1)
				{
						if(empty($value1['latitude']) || empty($value1['longitude']))
							 {
								 $value1['latitude']  = "0.00";
								 $value1['longitude'] = "0.00";
							 }
						 $arr=$value1;//encode
		...
		$array=array("Customer Details" => $cust_details,"Pick Boy Details"=>$arr);
		echo json_encode($array); 
	}
...
ini_set('max_execution_time', 0);
	...
	//---------------------------------------
	$fetchdataongoing=Yii::app()->db->createCommand("SELECT  distinct a.bookid,a.model_id,b.models_name,c.makes_name,a.makes_id,a.service_name,a.plan_name, a.amout,a.timestamp,a.pickadrs,e.car_img,a.status,max(a.timestamp),a.mech_status,a.delv_status,a.serviceon FROM MPS_PACKAGEWISE_AMT_DETAILS as a,MPS_VEHICLE_MODELS as b,MPS_VEHICLE_MAKES as c,MPS_CUSTOMER_INFO as mci,MPS_CUSTOMERACC_INFO as mcc,MPS_VEHICLES as e WHERE  b.models_id=a.model_id and c.makes_id=a.makes_id and mcc.loginid=a.userid and e.models_id=a.model_id and a.status=1 and a.mech_status=1  group by a.id")->queryAll();
	...
	//------------------------------------
	 ...
	 public function actionFetchPickBoyStatusDetails()
	 {
		 ...
		 $bookid=$_POST['bookid'];
	               ...
	               if(!empty($fetchdataDel)){
					   foreach($fetchdataDel as $value)
					 {
					
						 $arrPickBoyDetails['PickBoy Details'][]=$value;
					 }
					 $intCode = 200;
				   } 
					 $arrResponse = array('type'=>'success','code' => $intCode,'data' => $arrPickBoyDetails,'message' =>'PickupBoyDetails');
		 echo json_encode($arrResponse);
	...
	public function actionVehicleRepairLists()
	{
		...
		{   
			...
			$basicid=Yii::app()->db->createCommand("SELECT mcd.sname,mcd.id,count(mcd.id) as amount FROM `MPS_CARSERVICESLIST_DETAILS` as mcd,MPS_SUB_REPAIRLIST_DETAILS as msr where mcd.id=msr.repairid and msr.$value=$idd group by mcd.id")->queryAll();
				  ...
				  if(empty($rerepairids) || empty($sugrepairids))
				  {
					  $rec=0;
					  $sug=0;
