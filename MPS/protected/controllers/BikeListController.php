...
class BikeListController extends Controller
{
	...
	{
		$message='';
                $Brand_ImageSize=array(array('width'=> 60,'height' =>40));
                $ImageType='brand';
		if(isset($_FILES['bikelogofile']))
		{
			$arrImageFileInputs = array(
                            
                                                'Org_FileName' => $_FILES['bikelogofile']['name'],
						'Temp_FileName' => $_FILES['bikelogofile']['tmp_name'],
					);
                $retun_file_path=$this->actionUploadVehicleimages($arrImageFileInputs, $ImageType, $Brand_ImageSize);
                
                        if(!empty($retun_file_path))
                        {
                        $brand_name=$_POST['brand_name'];
			$model=new BikeBrands();
			$model->brand_name=$brand_name;
			$model->brand_logo_path=$retun_file_path;
			$model->save();
			$message="Added succesfully";
                     ...
                     $this->render('CreateBikeBrand',array("success"=>$message));
	}
		
	public function actionUploadVehicleimages($arrImageFileInputs, $ImageType, $ImageSizes)
        {
               
                 $valid_exts = array('jpeg', 'jpg', 'png', 'gif');
                 $max_file_size = 200 * 1024; #200kb
               
                    $ext = strtolower(pathinfo($arrImageFileInputs['Org_FileName'], PATHINFO_EXTENSION));
		//if (! $_FILES['image']['error'] && $_FILES['image']['size'] < $max_file_size) {
			if (in_array($ext, $valid_exts)) {
				
				if($ImageType != 'brand')
                                {
                                    $imagCount = count($ImageSizes)+1;
                                }
                                else
                                {
                                    $imagCount = count($ImageSizes);
                                }
				...
				for($i=0;$i<$imagCount;$i++)
					...
					if($ImageType == 'brand')
                                        {
                                            ...
                                            $path = $_SERVER['DOCUMENT_ROOT'].'/beena/mps/MPS/images/uploadimages/bikes/brands/' . uniqid() . '.'.$ext;
                                            $file_bike_path=uniqid() . '.'.$ext;
                                           
                                   
                                    
                                        }
                                        else if($ImageType == 'model' && (array_sum($ImageSizes[$i]) < 100)){
                                            
                                           
                                                $path = $_SERVER['DOCUMENT_ROOT'].'/beena/mps/MPS/images/uploadimages/bikes/models/' . uniqid() . '.'.$ext;
                                                $file_bike_path=uniqid() . '.'.$ext;
                                         ...
                                         else if($ImageType == 'model' && (array_sum($ImageSizes[$i]) > 100))
                                          {
                                                ...
                                                $path = $_SERVER['DOCUMENT_ROOT'].'/beena/mps/MPS/images/uploadimages/bikes/models/original/' . uniqid() . '.'.$ext;
                                                $org_bike_path=uniqid() . '.'.$ext;
                                          }
                                        
					
					
                                    $size = getimagesize($arrImageFileInputs['Temp_FileName']);
					...
					$h = 0 ? 0 : $size[1];
                                        ...
                                        array_push($ImageSizes,array('width' => $size[0],'height' => $size[1]));
                                        
					/* $ImageSizes = array_merge($ImageSizes,array('width' =>  $size[0],'height' => $size[1]));		
					print_r($ImageSizes);
					die(); */
                                                $arrImageInputs = array(
                                                'imageTempName' =>$arrImageFileInputs['Temp_FileName'],
						'imageSizeOne' => $ImageSizes[$i]['width'],
						'imageSizeTwo' => $ImageSizes[$i]['height'],
						'imagePath' => $path,
						'imageXCoordinate' => $x,
                                                'imageYCoordinate' => $y,
                                                'imageWidth' => $w,
	...
	public function actionCreateBikeModel()
	{
		...
		$message="";
                $Model_ImageSize=array(array('width'=> 60,'height' =>35));
		...
		{	
			...
			foreach($_POST['model_name'] as $m)
			{
                                $arrImageFileInputs = array(
                                                ...
                                                'Org_FileName' => $_FILES['bikemodelfile']['name'][$i],
						'Temp_FileName' => $_FILES['bikemodelfile']['tmp_name'][$i],
					);
                                $bike_model_file_path=$this->actionUploadVehicleimages($arrImageFileInputs, $ImageType, $Model_ImageSize);
				...
				$model1->brand_id=$brandid;
                                $model1->bike_model_img_path=$bike_model_file_path;
                                $model1->save();
				$i++;
				$message="Added succesfully";
			}
		 ...
		 $i=1;
                    ...
                    foreach ($bikelist as $self_detail) {
									...
									<img src="'.Yii::app()->request->baseUrl.'/'.$self_detail['brandlogo'].'" width="50px" / ></td>
												...
												<td>
													<a class = "btn btn-theme pull-right btnupdate" id="'.$self_detail['bike_model_id'].'*1" data-toggle = "modal" data-target = "#updateavailable">Update</a>
	...
	public function actionallBikeList()
        {
			...
			$services=Yii::app()->db->createCommand("SELECT bm.status,bm.bike_model_id,bb.brand_logo_path as brandlogo,bb.brand_name as brand_name,bm.bike_model_name as model_name from bike_brands as bb,bike_models as bm where bb.brand_id=bm.brand_id and bm.status=0 order by bb.brand_name asc")->queryAll();
		...
		public function actionupdateBikeLogo()
		{
				if(!empty($_POST['update']))
				{
					...
					$id=explode("*",$vid);
					$mid=$id[0];
					$bikebranid=Yii::app()->db->createCommand("SELECT brand_id from bike_models  where bike_model_id='$mid'")->queryAll();		
					$bid=$bikebranid[0]['brand_id'];
				if(isset($_FILES['bikemodel_file']['tmp_name']))
				{
				 	$model = BikeModels::model()->findByPk($id[0]);
					$path='/images/uploadimages/bikes/models/'.basename($_FILES['bikemodel_file']['tmp_name']);
					$idproof= Yii::app()->request->baseUrl;
					$url2=$_SERVER['DOCUMENT_ROOT']."$idproof/images/uploadimages/bikes/models/";
					$uploadfile1 = $url2 .basename($_FILES['bikemodel_file']['tmp_name']);
					move_uploaded_file($_FILES['bikemodel_file']['tmp_name'], $uploadfile1);
					$logos=file_get_contents($uploadfile1);
					$logoencrypted=base64_encode($logos);
					$model->bike_model_img_path=$path;					
					$model->update(); 
						
				}
				//var_dump($_FILES['brand_logo_path']['tmp_name']); exit;
					if(isset($_FILES['brand_logo_path']['tmp_name']))
					{
							$modela= BikeBrands::model()->findByPk($bid);
						$path1='/images/uploadimages/bikes/brands/'.basename($_FILES['brand_logo_path']['tmp_name']);
						
						$url=$_SERVER['DOCUMENT_ROOT']."$idproof/images/uploadimages/bikes/brands/";
						$uploadfile = $url .basename($_FILES['brand_logo_path']['tmp_name']);
						move_uploaded_file($_FILES['brand_logo_path']['tmp_name'], $uploadfile);
						$logos1=file_get_contents($uploadfile);
						$logoencrypted1=base64_encode($logos1);
						$modela->brand_logo_path=$path1;					
						$modela->update();
					}
				}
