...
/* echo "SELECT sfd.`id`,sfd.`vehicle_id`,sfd.`username`,sfd.`vehicle_category`,sfd.`vehicle_type`,sfd.`makes_id`,mvm.makes_name, mvmm.models_name,
		...
		sfd.`model_id`, sfd.`seating_capacity`,sfd.`price_per_hour`,sfd.`price`,sfd.`img_path`,sfd.`security_deposit`,sfd.`created_date`,sfd.`from_date`,
		sfd.`to_date`,sfd.`total_kms`,sfd.`extra_rate_per_kms`,sfd.`vehicle_features`,sfd.`variant` FROM `SLD_ADD_VEHICLE` as sfd,MPS_VEHICLE_MAKES as mvm,MPS_VEHICLE_MODELS as mvmm where sfd.makes_id=mvm.makes_id and sfd.model_id=mvmm.models_id and sfd.from_date <='1466643600' and
		sfd.to_date>='1467248400' and sfd.vehicle_type='car'")->queryAll();
		 ...
		 foreach($getdataself as $key=>$value)
			{
				...
				$data['SelfDriveCarsDetails'][]=$value;
	...
	public function actionManageUser()//select users
	{
		$rawData=Yii::app()->db->createCommand("SELECT mci.`id`, mci.`username`, mci.`surname`, concat(md.makes_name, md.models_name) as vehicle,mci.`emailid`, mci.`mobile_no` FROM `MPS_CUSTOMER_INFO` as mci,MPSVEHADDED_DETAILS as md where mci.id=md.user_id")->queryAll();
		
		$this->render('customers',array("customersdata"=>$rawData));
   ...
   'keyField' => 'shop_nm',
	...
	{     
		...
		if(isset($_POST['MPSUSER']))
		{
			...
			Yii::app()->session['username1']=$_POST['MPSUSER'];
...
if($imake==1){
	...
	public function actionExlusiveServiceInfo()
	{
		$servicenm=$_POST['veh_info'];
...
foreach($images as $image)
	...
	public function actionExclusiveService()//excecute 
	{
