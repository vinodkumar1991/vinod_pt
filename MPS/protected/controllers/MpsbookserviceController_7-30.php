...
class MpsbookserviceController extends Controller
{
	 ...
	 public function actionFetchServiceCompletedStatus()
	{
		...
		foreach($getcount2 as $value)
				...
				if($status > 0)
				{
					$arr=array("Response"=>"Finished","Status"=>null);
				}
				else{
					$arr=array("Response"=>"Finished","Status"=>1);
				}
	...
	public function actionFetchCustomerServiceDetails()
	{
		...
		foreach($fetchdata as $value)
				{
					$arr['Cust_Service'][]=$value;
					$status=$value['serviceon'];
				}
				 if($status < 1)
				{
				$SeviceTime=array("Cust_Service_Details"=>null);
				}
				else{
					$SeviceTime=array("Cust_Service_Details"=>$arr);
				}
				if(empty($SeviceTime)) 
				{
...
ini_set('max_execution_time', 0);
	...
	/* 	 $sqlCommand = 'SELECT dd.contact,a.delv_status,dd.delv_id,dd.shop_id,dd.del_nm,dd.latitude,dd.longitude  
FROM MPS_PACKAGEWISE_AMT_DETAILS as a
left join  delv_details as dd on dd.delv_id = a.delv_id
WHERE  
a.status=1 and a.mech_status=1 and a.delv_status=1 
and a.assignstatus=1 and a.delv_id=dd.delv_id and 
a.mechp_id=dd.shop_id and a.mechp_id="'.$mechp_id.'" and a.bookid="'.$bookid.'" and a.userid="'.$userid.'"';
$fetchdataDel=Yii::app()->db->createCommand($sqlCommand)->queryRow(); */
