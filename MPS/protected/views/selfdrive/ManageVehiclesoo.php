...
$(document).ready(function()
{
	//$( ".clickbtn" ).click(function() {
           $("#example").on("click", ".clickbtn", function(){
		...
		$('#hiddenvehicleid').val($(this).attr('id'));
		 ...
		 $.post('GetVehicleDetails',
						...
						function(response)
						{
					            ...
					            $('#startdate').val(dataParse['from_date']);
						    $('#enddate').val(dataParse['to_date']);
    ...
    $('#btnupdateveh').click(function()
         {
            id = $('#hiddenvehicleid').val();
            startdate = $('#startdate').val();
            enddate = $('#enddate').val();
            $.post("UpdateVehicleDetails",
						{
						   id:VehicleId,
                                                   from_date:startdate,
                                                   to_date:enddate,
						...
						function(response)
						{
			                            ...
			                            if(response > 0)
                                                    {
                                                        location.href="<?php echo Yii::app()->params['webURL'].'/SelfDrive/SelfAgent/SelfManageVehicles';?>";
                                                    }
						   else
                                                   {
                                                       location.href="<?php echo Yii::app()->params['webURL'].'/SelfDrive/SelfAgent/SelfManageVehicles';?>";
...
<?php 
                    ...
                    foreach ($VehicleDetails as $VehicleDetail) {
											   ...
											   <td>VEH'.$VehicleDetail['id'].'</td>
                                                                                           ...
                                                                                           if($VehicleDetail['avail_status'] < 1)
                                                                                           {
                                                                                               ?>
     ...
     <div id="updateavailable" class="modal fade">
				        <div class="modal-dialog">
				            <div class="modal-content">
							    ...
							    <form class="form-horizontal" action="UpdateVehicleDetails" method="POST">
							    <div class="row">
									<div class="form-group">
