...
$dataProvider=new CArrayDataProvider($rawData, array(
	...
	*/
	...
	$vlist=Yii::app()->db->createCommand("SELECT * FROM `MPS_VEHICLES` where category_id=$id and status=0")->queryAll();
		}else
		{
		...
		foreach($vlist  as $vlist)
	  	{
	       ...
	       $vresult[] = array('make_id'=>$res[0]['makes_id'],'models_id'=>$res[0]['models_id'],'ID'=>$vlist['id'],'makename'=>$res[0]['makes_name'],'modelname'=>$res[0]['models_name'],'year'=>$vlist['year'],'logo'=>$vlist['logo_img'],"status"=>$res[0]['status']);

	  	}
		if($_GET['id']==1)
		{
			$data=Yii::app()->db->createCommand("SELECT `id`, `categoryname` FROM `MPS_VEHICLE_CAT` WHERE `id` in(1,5,9)")->queryAll();
		}
		else if($_GET['id']==2)
		{
			$data=Yii::app()->db->createCommand("SELECT `id`, `categoryname` FROM `MPS_VEHICLE_CAT` WHERE `id` in(2,6,10)")->queryAll();
		}
		else if($_GET['id']==3)
		{
			$data=Yii::app()->db->createCommand("SELECT `id`, `categoryname` FROM `MPS_VEHICLE_CAT` WHERE `id` in(3,7,11)")->queryAll();
		}
		else if($_GET['id']==4)
		{
			$data=Yii::app()->db->createCommand("SELECT `id`, `categoryname` FROM `MPS_VEHICLE_CAT` WHERE `id` in(4,8,12)")->queryAll();
		}
