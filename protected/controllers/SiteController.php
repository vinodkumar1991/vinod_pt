...
class SiteController extends Controller {
    ...
    public function actionFetchBookingDetails() {
        ...
        $fetchdata = Yii::app()->db->createCommand("select distinct c.models_name,c.imgpath,d.pkname,msd.servicenames from MPSVEHADDED_DETAILS as c,
										MPS_SERVICE_PACKAGE_DETAILS as d,MPS_PACKAGEWISE_AMT_DETAILS
										as mpd,MPS_SERVICES_DETAILS as msd where d.packageid=mpd.planid and mpd.serviceid=msd.id and  mpd.userid=2")->queryAll();
        $arr = array();
        foreach ($fetchdata as $key => $value) {
            $arr['Boking Details'][] = $value; //encode
        }
        print_r($arr);
    }
