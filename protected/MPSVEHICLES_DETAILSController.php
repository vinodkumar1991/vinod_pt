...
class MPSVEHICLES_DETAILSController extends Controller
{
	...
	public function actionBookingDetails()
	{
		 ...
		 $getmodelnm=Yii::app()->db->createCommand("SELECT  `car_img`, `veh_type` FROM `MPS_VEHICLES` WHERE `models_id`=$model_id")->queryAll();
	...
	public function actionFetchRepairListsRepairJob()
					...
					foreach($basicid as $basici)
					{
						...
						if(count($getcount)>0)
						{
						 $html.= '<div class="col-md-4">
                                      <h3 class="block-title"><input type="checkbox" id="chk2'.($id+1).'chk2'.$pkid.'" value="'.$id.'-'.$amt.'-'.$pkid.'" 
									  name="check" class="checkbox_check" checked/>'.$basici['sname'].'</h3>
		...
		public function actionFetchBikegenServiceDetails()
		{
				...
				$basicid=Yii::app()->db->createCommand("SELECT repair_id, repair_name, service_id FROM mps_bike_repair_lists where service_id=1")->queryAll();
					...
					foreach($basicid as $basici)
					{
						...
						$html.= '<div class="col-md-4">
                                      <h3 class="block-title"><input type="checkbox" name="chk'.$id.'" id="chk'.$id.'"/>'.$basici['repair_name'].'</h3>
										<ul class="list-check">';
					
						$basiclists=Yii::app()->db->createCommand("SELECT bsr.sub_repair_id, bsr.sub_name FROM bike_sub_repairlists as bsr,mps_bike_repair_lists as mbr where bsr.repair_id=mbr.repair_id and mbr.repair_id=$id")->queryAll();
		...
		public function actionFetchBikerepairServiceDetails()
		{
				...
				$html='<script src="'.Yii::app()->baseUrl.'/assets/plugins/jquery/jquery-1.11.1.min.js"></script>
				<input type="text" name="amount" id="amount'.$model_id.'"/>
				';
				$basicid=Yii::app()->db->createCommand("SELECT repair_id, repair_name, service_id FROM mps_bike_repair_lists where service_id=2")->queryAll();
					...
					{
						$id=$basici['repair_id'];
						$getsub=Yii::app()->db->createCommand("SELECT distinct sub_repair_id FROM `bike_repair_package_list` WHERE repair_id=$id")->queryAll();
					
					foreach($getsub as $getsu)
						{
							$sid=$getsu['sub_repair_id'];
							
							$getsum=Yii::app()->db->createCommand("SELECT sum(a.amount) as amount from bike_repair_package_list as a where  a.repair_id and a.repair_id = $id 
							and a.sub_repair_id= $sid and
							  a.category_id=$category_id")->queryAll();
							
							foreach($getsum as $amout1)
							{
								 $amt=$amout1['amount'];
						...
						$html.= '<div class="col-md-4">
                                      <h3 class="block-title"><input type="checkbox" name="chk'.($id+1).'" id="chk'.($id+1).'" value="'.$amt.'"/>'.$basici['repair_name'].'</h3>
										<ul class="list-check">';
					
						$basiclists=Yii::app()->db->createCommand("SELECT bsr.sub_repair_id, bsr.sub_name FROM bike_sub_repairlists as bsr,mps_bike_repair_lists as mbr where bsr.repair_id=mbr.repair_id and mbr.repair_id=$id")->queryAll();
						
						$basicamts=Yii::app()->db->createCommand("SELECT sum(a.amount) as amounts from bike_repair_package_list as a where  a.repair_id and a.repair_id = $id and  a.category_id=$category_id")->queryAll();
					
						$totalbasicamt=0;
						foreach($basicamts as $basicamt)
						{
							//echo $basiclist['subvalue'];
							 $basicamtt = $basicamt['amounts'];
			...
			$basicid=Yii::app()->db->createCommand("SELECT mcd.sname,mcd.id,count(mcd.id) FROM `MPS_CARSERVICESLIST_DETAILS` as mcd,MPS_SUB_REPAIRLIST_DETAILS as msr where mcd.id=msr.repairid and msr.advanced=3 group by mcd.id")->queryAll();
					...
					foreach($basicid as $basici)
					{
					 ...
					 foreach($advanceamts as $advance)
						...
						echo  $html.'**'.'Rs.'.$advanceamt.'**'.$servhr;
					//echo  $html;
		...
		public function actionFetchRepairListsPeriodic()
		{
			...
			foreach($GETCATEIDs as $GETCATEID)
					...
					if(count($fetchrepiarid)>0)
					{
						    ...
						    foreach($fetchrestatus as $fetchres)
							{
								 $repid=$fetchres['id'];
							}
					}
				...
				$basicamts=Yii::app()->db->createCommand("SELECT sum(a.amount) as amount from repairlist_package_details as a,MPS_RECOMENDED_SERVICE as b where  b.repid=a.repair_id and a.repair_id in ($repid) and b.pkid=$pkid and a.category_id=$category_id")->queryAll();
				 ...
				 as mcd,MPS_SUB_REPAIRLIST_DETAILS as msr where mcd.id=msr.repairid and msr.$pknm=$pkid group by mcd.id")->queryAll();
					...
					$(document).ready(function()
					{
						...
						$("#disamt'.$pkid.'").val('.$planamt.');
						
						model_id=$("#model_id").val();
						plan='.$pkid.';
						
						$("#chk1'.($id+1).'chk1'.$pkid.'").change(function(){
							if($(this).prop("checked")==true)
							{
				...
				echo  $html.'**'.'Package:'.($basicamt+$planamt).'**'.$servhr.'**'.$pkid.'**'.($basicamt+$planamt);		//echo  $html;
		...
		public function actionUncheckInput()
		{
				...
				if(count($getcount)>0)
				{
					//echo count($getcount);
					 ...
					 and planid=$planid and categoryid=$categoryid")->execute();
					echo 'updated';
				}
			...
			if(empty(Yii::app()->session['username']))
			{
				 ...
				 if(count($repairids)<2)
				  ...
				  { 
					    ...
					    $getcount2=Yii::app()->db->createCommand("update MPS_PACKAGEWISE_AMT_DETAILS set userid=$userid,amout = $usertot,model_id=$model_id,makes_id=$makes_id WHERE repairid=$repairid and planid=$planid")->execute();
				  } 
					 }
				   ...
				   $userid = $datas[2];
					 ...
					 if(count($repairids)<2)
					 {
					  ...
					  { 
							...
							$getcount2=Yii::app()->db->createCommand("update MPS_PACKAGEWISE_AMT_DETAILS set total=$userid,model_id=$model_id,amout = $usertot,makes_id=$makes_id WHERE repairid=$repairid and planid=$planid")->execute();
					  } 
...
//		exit;
	...
	{   
		...
		if(!empty(Yii::app()->session['username']))
		{
			...
			//echo Yii::app()->session['lastid'];
							...
							where MPS_VEHICLE_MODELS.models_id=MPSVEHADDED_DETAILS.model_id and MPSVEHADDED_DETAILS.user_id=".Yii::app()->session['lastid']."  ")->queryAll();
							//$getservices=Yii::app()->db->createCommand("SELECT `id`, `servicenames` FROM `MPS_SERVICES_DETAILS`")->queryAll();
							
							//$vmodel=Yii::app()->db->createCommand("SELECT models_id,`models_name` FROM `MPSVEHADDED_DETAILS` WHERE user_id=$lastid")->queryAll();
	...
	public function actionFetchmodelImage()
	{
		  ...
		  $modelid = $_POST['modelid'];
			...
			$datas=Yii::app()->db->createCommand("SELECT MPSVEHADDED_DETAILS.imgpath,MPS_VEHICLE_MAKES.makes_name,MPS_VEHICLE_MODELS.models_name FROM `MPSVEHADDED_DETAILS`,MPS_VEHICLE_MAKES,MPS_VEHICLE_MODELS WHERE  MPS_VEHICLE_MODELS.models_id=MPSVEHADDED_DETAILS.model_id and MPS_VEHICLE_MAKES.makes_id=MPSVEHADDED_DETAILS.makes_id and MPS_VEHICLE_MODELS.models_id=$modelid")->queryAll();
	...
	public function actionGetvmodel()
	{
		...
		for($i=0;$i<count($models_id);$i++)
		{
			...
			$carimagesdata=Yii::app()->db->createCommand("SELECT `models_id`,`car_img` FROM `MPS_VEHICLES` WHERE makes_id=$makes_ids[$i] and models_id=$models_id[$i]")->queryAll();
			if(!empty($carimagesdata))
			{
				...
				$html.='<li id="'.$models_id[$i].'"><a href="#">'.$models_name[$i].'<img src="http://10.10.10.28/mps/MPS/images/uploadimages/models/car/default_car.jpg"  height="35" width="35"></a></li>';
			}
