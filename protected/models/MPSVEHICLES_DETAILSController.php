...
class MPSVEHICLES_DETAILSController extends Controller
{
	...
	public function actionOrderReceived()
	{
		...
		if(isset($_POST['finalsubtotal']))
		{
			...
			if($gettransaction[0]['serviceid']==1)
			{
				$typeofservice="General Service";
			}else if($gettransaction[0]['serviceid']==2)
			{
				$typeofservice="Common Service";
			}
		...
		$this->render('order-received',array("bookid"=>$bookid,"vehicle_type"=>$vehicle_type,"typeofservice"=>$typeofservice,"amount"=>$amount));
	}
	...
	public function actionBikeServiceOrderSummary1()
	{
		 ...
		 if(!empty($_POST['order']))
		{
			...
			$model->amout=$amount;
			$model->save();	
			$this->redirect(array('BikeServiceOrderSummary','id'=>$model->bookid));
		...
		public function actionBikeServiceOrderSummary($id)
	{
		...
		$get_transaction_details=Yii::app()->db->createCommand("SELECT username,emailid,mobile_no FROM MPS_CUSTOMER_INFO WHERE id='$userid'")->queryAll();
		$emailid=$get_transaction_details[0]['emailid'];
		$mobile=$get_transaction_details[0]['mobile_no'];
		$username=$get_transaction_details[0]['username'];
	
		$this->render('bikeserviceBilling',array("bookid"=>$bookid,"amount"=>$amount,"date"=>$pickdate,"hr"=>$pickhr,"adress"=>$pickadrs,"email"=>$emailid,"username"=>$username,"mobile"=>$mobile));
	}
	...
	public function actionFetchRepairListsRepairJob()
		...
		public function actionBookBikeSummary()
		{
		 ...
		 if($bserviceid==1)
			...
			$this->render('bookService_Summary',array("date"=>$date,"adrs1"=>$adrs1,"time"=>$time,"modelid"=>$modelid,"service"=>$service,"servicecat"=>$service_category,"bikedetails"=>$bikedetails,"html"=>$html,"service_cat"=>$scat,"total_amount"=>$total_amount));
		}
		...
		public function actionCarDetails()
		{
			...
			"picdate"=>$picdate,"pickhr"=>$pickhr,"planid"=>$planid,"service_id"=>$service)); 
		...
		if(count($getcount2)>0)
		{
			...
			$this->render('order-received',array("bookid"=>$bookid,"vehicle_type"=>'Car',"typeofservice"=>$service_name,"amount"=>$amount));
