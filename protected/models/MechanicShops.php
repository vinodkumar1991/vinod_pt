...
class MechanicShops extends CActiveRecord {
    ...
    public static function mechanicShopsReport($arrInputs = array()) {
        ...
        $objectDB->select('ms.id,ms.name as shop_name,ms.owner as shop_owner,ms.license as shop_license,'
                ...
                . 'u.username,u.first_name,ms.code as shop_code,group_concat(st.name) as service_names,msd.total_mechanics,msd.service_capability,ms.status,group_concat(CONCAT("""", st.id, """")) as service_ids,msd.location,msd.latitude,msd.longitude,mss.vehicle_types_id as vehicle_id'
        );
        $objectDB->from('mechanic_shops as ms');
    ...
    public static function getMechanicShops($intVehicle, $intService, $intStatus = 1, $arrLatiLongis) {
        $arrShops = array();
        try {
            $strQuery = 'SELECT  ms.id as shop_id,ms.name as shop_name,ms.owner as shop_owner,ms.code as shop_code,msd.latitude,msd.longitude,ms.email,ms.phone,msd.location as address,u.gcm_register_id
                     ...
                     left join users as u on u.id = ms.users_id';
    ...
    public function getDeliveryBoys($intShop) {
        ...
        $intStatus = 1;
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->select('u.id as delivery_boy_id,db.name as delivery_boy_name,u.role_types_id as role_id,ms.id as shop_id,ms.name as shop_name,ms.code as shop_code,ms.owner as shop_owner,mss.vehicle_types_id as vehicle_id,vt.name as vehicle_type,db.email as delivery_boy_email,db.phone as delivery_boy_phone,msd.location,msd.latitude,msd.longitude');
        ...
        $objectDB->group('db.id');
        $arrDeliveryBoys = $objectDB->queryAll();
        return $arrDeliveryBoys;
    }

    public static function updateMechanicStatus($intShop, $arrMechanic) {
        $objCommand = Yii::app()->db->createCommand();
        $intUpdate = $objCommand->update('mechanic_shops', $arrMechanic, 'id=:id', array(':id' => $intShop));
        return $intUpdate;
    }

    public static function getNetWork($arrMinMaxLatiLongis = array()) {
        $arrNetwork = array();
        $strQuery = 'select ms.name as shop_name,msd.location,msd.latitude,msd.longitude
                        from mechanic_shops as ms
                        left join mechanic_shop_details as msd on msd.mechanic_shops_id = ms.id where 1 ';
        if (!empty($arrMinMaxLatiLongis)) {
            $strQuery .= ' and msd.latitude between "' . $arrMinMaxLatiLongis['min_lati'] . '" and "' . $arrMinMaxLatiLongis['max_lati'] . '"';
            $strQuery .= ' and msd.longitude between "' . $arrMinMaxLatiLongis['min_longi'] . '" and "' . $arrMinMaxLatiLongis['max_longi'] . '"';
        }
        $strQuery .= ' order by ms.name ASC';
        $arrNetwork = Yii::app()->db->createCommand($strQuery)->queryAll();
        return $arrNetwork;
    }
