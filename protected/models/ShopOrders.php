...
class ShopOrders extends CActiveRecord {
    ...
    public static function create($arrShopOrders) {
        $intShopOrder = NULL;
        $objShopOrder = new ShopOrders();
        $objShopOrder->shop_id = $arrShopOrders['shop_id'];
        $objShopOrder->order_id = $arrShopOrders['order_id'];
        $objShopOrder->status = $arrShopOrders['status'];
        $objShopOrder->created_date = $arrShopOrders['created_date'];
        $objShopOrder->created_by = $arrShopOrders['created_by'];
        $objShopOrder->ip_address = $arrShopOrders['ip_address'];
        $objShopOrder->device_id = $arrShopOrders['device_id'];
        $objShopOrder->imei_no = $arrShopOrders['imei_no'];
        if ($objShopOrder->save()) {
            $intShopOrder = $objShopOrder->id;
        }
        return $intShopOrder;
    }
     ...
     */
    public function shopOrdersReport($intShop, $intStatus = 1, $arrShop = array()) {
        $arrShopOrders = array();
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->select('so.id');
        $objectDB->from('shop_orders as so');
        $objectDB->where('so.shop_id=:shopId and so.status=:status', array(':shopId' => $intShop, ':status' => $intStatus));
        if (!empty($arrShop)) {
            $objectDB->andWhere('so.is_accepted=:isAccepted and so.is_completed=:isCompleted', array(':isAccepted' => $arrShop['isAccepted'], ':isCompleted' => $arrShop['isCompleted']));
        }
        $arrShopOrders = $objectDB->queryAll();
     ...
     * @return array It will return shop details of an order
     */
    public function getShopDetails($intOrder) {
        ...
        $objectDB->select('ms.id as shop_id,ms.name as shop_name,ms.owner as shop_owner,msd.location as shop_location,'
                ...
                . 'o.order_number,vbm.image_path as model_logo,(CASE WHEN  o.vehicle_types_id = 1 THEN "/cars/mobile/models/original/" ELSE "/bikes/mobile/models/original/" END) AS model_path,(CASE WHEN o.vehicle_variant_id = 1 THEN "Petrol" ELSE "Diesel" END) AS vehicle_variant,
        ...
        $objectDB->from('shop_orders as so');
        $objectDB->join('orders as o', 'o.id = so.order_id');
    ...
    public function assignOrderToBoy($intShop, $intOrder, $intDeliveryBoy = NULL, $arrDeliveryBoy = array()) {
        ...
        if (empty($intDeliveryBoy)) {
            $intUpdate = $objCommand->update('shop_orders', $arrDeliveryBoy, 'shop_id=:shopId and order_id=:orderId', array(':orderId' => $intOrder, ':shopId' => $intShop));
        } else if (!empty($intDeliveryBoy)) {
            $intUpdate = $objCommand->update('shop_orders', $arrDeliveryBoy, 'shop_id=:shopId and order_id=:orderId and delivery_boys_id=:deliveryBoyId', array(':orderId' => $intOrder, ':shopId' => $intShop, ':deliveryBoyId' => $intDeliveryBoy));
        }
        return $intUpdate;
    }
    ...
    public function getOrderStatus($intOrder, $intStatus) {
        ...
        $objectDB->select('o.id,msd.location,msd.latitude,msd.longitude,o.completed_time as completedTime,concat_ws("",DATE_FORMAT(o.completed_date,"%b %d %Y"),", ",o.completed_time) as completed_time,
                          (CASE WHEN o.order_status = 1 THEN "NEW"
                           WHEN o.order_status = 2 THEN "ACCEPTED"
                           WHEN o.order_status = 3 THEN "REJECTED"
                           WHEN o.order_status = 4 THEN "ASSIGNED"
    ...
    public function isDeliveryBoyHaveOrders($intDeliveryBoy) {
        $intActive = 1;
        $intInactive = 0;
        $strQuery = 'select so.id from shop_orders as so where so.delivery_boys_id = "' . $intDeliveryBoy . '"
    ...
    public function isAssigned($intShop, $intOrder, $intRunner = 0) {
        ...
        $intStatus = 1;
        $objectDB = Yii::app()->db->createCommand();
        $objectDB->select('so.id,so.is_accepted,so.is_collected,so.is_completed,so.is_started,so.is_repairs_completed,so.is_repair_delivery_boy,o.order_status,u.gcm_register_id');
        $objectDB->from('shop_orders as so');
        $objectDB->leftJoin('orders as o', 'o.id = so.order_id');
        $objectDB->leftJoin('users as u', 'u.id = so.delivery_boys_id');
        if (!empty($intRunner)) {
            $objectDB->where('so.shop_id=:shopId and so.status=:status and so.order_id=:orderId and so.delivery_boys_id=:deliveryBoyId', array(':shopId' => $intShop, ':status' => $intStatus, ':orderId' => $intOrder, ':deliveryBoyId' => $intRunner));
        } else {
            $objectDB->where('so.shop_id=:shopId and so.status=:status and so.order_id=:orderId', array(':shopId' => $intShop, ':status' => $intStatus, ':orderId' => $intOrder));
        }
    ...
    public function updateOrderStatus($intOrder, $arrOrder) {
        $objCommand = Yii::app()->db->createCommand();
        $intUpdate = $objCommand->update('shop_orders', $arrOrder, 'order_id=:orderId', array(':orderId' => $intOrder));
        return $intUpdate;
    }
    ...
    public function shopOrdersStatus($intOrder, $intRunner = 0) {
        ...
        $objectDB->from('shop_orders as so');
        if (!empty($intRunner)) {
            $objectDB->where('so.order_id=:orderId and so.delivery_boys_id=:runnerId', array(':orderId' => $intOrder, ':runnerId' => $intRunner));
        } else {
            $objectDB->where('so.order_id=:orderId', array(':orderId' => $intOrder));
        }
        $arrShopOrders = $objectDB->queryRow();
