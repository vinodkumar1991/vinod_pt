...
class VehicleServiceTypes extends CActiveRecord
{
    ...
    public static function getServiceTypes($intStatus = 1, $intVehicleType, $intSign = NULL)
    {
        ...
        if (empty($intSign)) {
            $objectDB->where('vst.status=:status and vst.vehicle_types_id=:vehicleTypeId', array(
                ':status' => $intStatus,
                ':vehicleTypeId' => $intVehicleType
            ));
        } else {
            $objectDB->where('vst.status=:status and vst.vehicle_types_id=:vehicleTypeId and st.id!=:serviceTypeId', array(
                ':status' => $intStatus,
                ':vehicleTypeId' => $intVehicleType,
                ':serviceTypeId' => 4
            ));
