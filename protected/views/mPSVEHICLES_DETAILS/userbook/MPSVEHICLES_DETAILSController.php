...
class MPSVEHICLES_DETAILSController extends Controller
{
	...
	{
		$this->render('bikeserviceBilling');
	}
	...
	public function actionFetchRepairListsRepairJob()
		...
		public function actionCarDetails()
		{
			...
			$post_brand_nm=$_POST['post_brand_nm'];
			$post_mod_nm=$_POST['post_mod_nm'];
			
			$post_brand_id=$_POST['post_brand_id'];
			$post_mod_id=$_POST['post_mod_id'];
			
			$mod_path=$_POST['mod_path'];
			$payamount=$_POST['payamount'];
			$data=explode("Package:",$payamount);
			$sernm=$_POST['sernm'];
			...
			$hidval=$_POST['hidval'];
			  ...
			  $fetchmps=Yii::app()->db->createCommand("SELECT MPS_CARSERVICESLIST_DETAILS.`sname` FROM `MPS_RECOMENDED_SERVICE`,MPS_CARSERVICESLIST_DETAILS  WHERE MPS_RECOMENDED_SERVICE.`pkid`=$planid and MPS_CARSERVICESLIST_DETAILS.id=MPS_RECOMENDED_SERVICE.repid")->queryAll(); 
			
			
			  $fetch_select_details=Yii::app()->db->createCommand("SELECT MPS_CARSERVICESLIST_DETAILS.sname FROM MPS_PACKAGEWISE_AMT_DETAILS,MPS_CARSERVICESLIST_DETAILS WHERE  MPS_PACKAGEWISE_AMT_DETAILS.`repairid`=MPS_CARSERVICESLIST_DETAILS.id and status=1 and  MPS_PACKAGEWISE_AMT_DETAILS.planid=$planid and sess_id='$sees_id'")->queryAll(); 
			
		
				
			
			$this->render('user_book',array("post_brand_nm"=>$post_brand_nm,"post_mod_nm"=>$post_mod_nm,"mod_path"=>$mod_path,"payamount"=>$data[1],"servicename"=>$sernm,"planname"=>$plannm,"fetch_select_details"=>$fetch_select_details,"post_brand_id"=>$post_brand_id,"post_mod_id"=>$post_mod_id,"fetchmps"=>$fetchmps,"planid"=>$planid,"serviceid"=>$serid,
			"hidvalue"=>$hidval));
			}
			else{
				$post_brand_nm=$_POST['post_brand_nm'];
				...
				$GETCATEIDs=Yii::app()->db->createCommand("SELECT  `category_id` FROM `MPS_VEHICLES` WHERE models_id=$post_mod_id")->queryAll();
				foreach($GETCATEIDs as $GETCATEID)
						{
							$category_id=$GETCATEID['category_id'];
						} 
			    ...
			    $amtvalue='';
				$basicid=Yii::app()->db->createCommand("SELECT mcd.sname,mcd.id,count(mcd.id) FROM `MPS_CARSERVICESLIST_DETAILS` as mcd,MPS_SUB_REPAIRLIST_DETAILS as msr where mcd.id=msr.repairid and msr.$plannm=$planid group by mcd.id")->queryAll();
					
				$this->render('user_book',array("post_brand_nm"=>$post_brand_nm,"post_mod_nm"=>$post_mod_nm,"mod_path"=>$mod_path,"payamount"=>$data[1],"servicename"=>$sernm,"planname"=>$plannm,"post_brand_id"=>$post_brand_id,"post_mod_id"=>$post_mod_id,"planid"=>$planid,"serviceid"=>$serid,"basicid"=>$basicid));
			...
			$this->render('user_book',array("brand_name"=>$post_brand_id,"model_name"=>$post_mod_id,"html"=>$html,"sernm"=>$sernm,"img_path"=>$mod_path,"payamount"=>$payamount,"servicename"=>$ser_name,"planname"=>$plannm));
